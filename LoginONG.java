...
public class LoginONG {
    ...
    private void initialize() {
        ...
        JLabel labelTitulo = new JLabel("Faça login como Organização não Governamental:");
        constraints.gridx = 0;
        constraints.gridy = 1;
        constraints.gridwidth = 2;
    ...
    private void realizarLogin() {
        ...
        String query = "SELECT * FROM ong WHERE email = ? AND senha = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (resultSet.next()) {
                int idOngLogado = resultSet.getInt("idOng");

                // Armazenar o ID do ong logado na classe UserData
                UserData userData = UserData.getInstance();
                userData.setIdOngLogado(idOngLogado);

                JOptionPane.showMessageDialog(frame, "Login realizado com sucesso!");
                fecharJanela(); // Fecha a janela de login

                // Abrir a interface ong
